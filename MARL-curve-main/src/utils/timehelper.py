...
def print_time(start_time, T, t_max, episode, episode_rewards):
    """Print out information with the time information"""
    # DS: calculate time_left from time_elapsed & T & t_max
...
def time_left(start_time, t_start, t_current, t_max):
    ...
    if t_current >= t_max:
        return "-"
    # DS: calculate time_left from time_elapsed & t_current & t_max
    time_elapsed = time.time() - start_time
